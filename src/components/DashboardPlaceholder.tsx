    ...
    <div className="flex flex-col items-center max-w-4xl mx-auto pt-6 md:pt-12 w-full gap-6 md:gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 w-full max-w-3xl mb-8">
        <button 
          ...
          className={`flex flex-col items-start p-5 md:p-6 rounded-xl border transition-all text-left shadow-sm hover:shadow-md hover:border-foreground/20 ${showCourseUpload ? 'border-primary ring-1 ring-primary' : 'border-border bg-card'}`}
        >
...
function BetaLimitsNotice({limits, voiceLimits}: {limits: BetaLimits; voiceLimits?: DashboardVoiceLimits}) {
  ...
  return (
    <section className="w-full max-w-3xl rounded-lg border border-border bg-card px-4 py-3 text-left shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        {limits.frozen && (
          <span className="w-fit rounded-md bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
            {t('beta.paused')}
          </span>
        )}
      </div>
...
function ActionCard({
  ...
  return (
    <button className={`flex flex-col items-start p-5 md:p-6 rounded-xl border transition-all text-left relative overflow-hidden ${comingSoon ? 'border-border/50 bg-muted/50 cursor-not-allowed opacity-70 hover:opacity-100 group' : 'border-border bg-card shadow-sm hover:shadow-md hover:border-foreground/10'}`}>
      ...
      {comingSoon && (
        <div className="absolute top-4 right-4 rotate-12 transition-transform group-hover:scale-110">
          <span className="bg-muted text-muted-foreground text-xs font-bold px-2 py-0.5 rounded shadow-sm border border-border">
            SOON
          </span>
        </div>
      )}
